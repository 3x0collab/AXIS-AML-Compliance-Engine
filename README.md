...
Because this system processes sensitive financial data and contains proprietary risk algorithms, **the source code is not public.** This repository serves as documentation of the architecture, the problems I solved, and the UI workflow.
...
1.  **Request:** External service sends a transaction payload via API.
2.  **Validation:** AXIS validates the schema and checks the user's current status.
3.  **The Logic Layer:**
...
4.  **Verdict:** The system returns a `PASS`, `BLOCK`, or `REVIEW` status instantly.

### Dashboard Preview
*(This is the internal tool compliance officers use to view flagged/held transactions)*

![AXIS Dashboard](Screenshot%202025-08-08%20122734.png)
